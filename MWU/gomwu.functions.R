...
gomwuStats_cut=function(input,goDatabase,goAnnotations, goDivision, Module=FALSE, 
                        Alternative="t", adjust.multcomp="BH", clusterCutHeight=0.25,largest=0.1,
                        smallest=5,perlPath="perl", shuffle.reps=20, iteration = 0, slope=0){
  inname=paste(goDivision,"_",input,sep="")	
  rsq=read.table(inname,sep="\t",header=T)
  
  seed <- as.integer(runif(1, 0, 500000))
  set.seed(seed)
  
  # custom iteration
  orig_val <- read.table(input, header = T, sep=",")
  
  # for iteration = 1 we assign random pvalues/fold changes from the original values
  if(iteration == 1){
    cat("Iterating values over transcripts \n")
    orig_val$random_fc <- sample(orig_val$stat)
    for(i in unique(rsq$seq)){
      # print(i)
      rsq$value[rsq$seq==i] <- orig_val$random_fc[orig_val$id == i]
    }
  } else if(iteration == 2){
    cat("Iterating GO terms over transcripts \n")
    taken <- c()
    `%notin%` <- Negate(`%in%`)
    terms <- rsq$term
    for(i in unique(terms)){
      indices <- which(terms ==i)
      random_term <- sample(unique(terms[terms != i & terms %notin% taken]), 1)
      rsq$term[indices] <- random_term
      taken <- c(taken, random_term)
    }
  } else if(iteration == 3){
    cat("Sampling values from a normal distribution with m=0 and sd(log2FC) \n")
    orig_val$random_fc <- rnorm(length(orig_val$stat), 0, sd(orig_val$stat))
    for(i in unique(rsq$seq)){
      rsq$value[rsq$seq==i] <- orig_val$random_fc[orig_val$id == i]
    }
  } else if(iteration == 4){
    ...
    # split data randomly
    x_indices <- sample(c(1:length(orig_val$stat)), as.integer(length(orig_val$stat)*percentage/100))
    x_big <- orig_val[-x_indices,]
    x_big$special <- 0
    x_small <- orig_val[x_indices,]
    x_small$special <- 1
    
    # assign norm values
    x_big$stat <- rnorm(length(x_big$stat), 0, var(orig_val$stat))
    x_small$stat <- rnorm(length(x_small$stat), mean.special, var(orig_val$stat))
    
    x <- rbind(x_big, x_small)
    
    for(i in unique(rsq$seq)){
      rsq$value[rsq$seq==i] <- x$stat[x$id == i]
    }
  } else if(iteration == 5){ # for lm as mean function
    # load models
    load("models.RData")
    # get number of GO per transcript
    n <- as.vector(sapply(unique(rsq$seq), function(x) length(which(x==rsq$seq))))
    # sample depending on n
    fc <- rnorm(length(n), mean=predict(real_lm, data.frame(nmemberlog10=log10(n))),
                sd=rnorm(1, mean=.9, sd=.01*exp(1.5*log10(n))))
    new <- as.data.frame(cbind("seq"=unique(rsq$seq),
                               "n"=n,
                               "fc"=fc))
    ...
    # reassign to rsq
    for(i in unique(rsq$seq)){
      rsq$value[rsq$seq==i] <- new$fc[new$seq==i]
    }
    
  } else if(iteration == 6){ # for loess as mean function
    load("models.RData")
    # get number of GO per transcript
    n <- as.vector(sapply(unique(rsq$seq), function(x) length(which(x==rsq$seq))))
    if (any(n > 654)){
      return(cat("Aborting the run, because the loess model can only take GO terms till a size of 656 transcripts. It has not been trained for more"))
    }
    # sample depending on n
    fc <- rnorm(length(n), mean=predict(lo, log10(n)),
                sd=rnorm(1, mean=.9, sd=.01*exp(1.5*log10(n))))
    new <- as.data.frame(cbind("seq"=unique(rsq$seq),
                               "n"=n,
                               "fc"=fc))
  ...
  } else if(iteration == 7){ # for sampling on varying slope
    intercept <- 1.24221*slope
    # get number of GO per transcript
    n <- as.vector(sapply(unique(rsq$seq), function(x) length(which(x==rsq$seq))))
    # sample depending on n
    fc <- rnorm(length(n), mean=slope*log10(n)-intercept, sd=0.9192379)
    ...
    cat("No valid iteration procedure was defined. Aborting.\n")
    break
  }
  ...
  if(iteration == 4){
    return(list("result" = rr, "runinfo" = list("seed"=seed, "mean" = mean.special, "percentage" = percentage), "data" = x, "goinfo"=rsq))
  }
  return(list("result" = rr, "seed" = c(seed, slope), "goinfo"=rsq))
}
