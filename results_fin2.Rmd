...
load("./data/RMD_orig.RData")
load("./data/orig_x.RData")

```

```{r}
orig_fc_o_trans <- orig_fc_o_trans[-which(orig_fc_o_trans$method %in% c("elim", "weight", "lea") & orig_GO_N$fdr == "fdr"),]
orig_fc_o_trans <- orig_fc_o_trans[-which(orig_fc_o_trans$N == 0),]

orig_GO_N_o_t <- orig_GO_N_o_t[-which(orig_GO_N_o_t$method %in% c("elim", "weight", "lea") & orig_GO_N_o_t$fdr == "fdr"),]
orig_GO_N_o_t <- orig_GO_N_o_t[-which(orig_GO_N_o_t$N_sig == 0),]
...
# par(mfrow=c(2,4))
# for(j in 1:8){
# plot(x=1:300, y=sapply(1:300, function(x) median(orig_mwu[[j]]$trans[,1][which(orig_mwu[[j]]$misc$trans$GO_N >= x)], na.rm=T)), type="l", ylim=c(-1, 1), xlab="Number of annotated GO terms", ylab="Cumulative Median log2FC", main=paste0(buchstab[j], ") ", "slope: ", slope[j]))
# for(i in 2:100){
...
mean <- NULL
for(i in 1:8){
  mean <- rbind(mean, cbind(mean2 = apply(orig_results[[i]]$classic$trans, 2, median),
                slope = slope[i]))
}
mean <- as.data.frame(mean)

boxplot(mean$mean2~mean$slope, main="Median of Random Sampling iteorigions over slope \n whole annotations", ylab="Median log2 Fold Change", xlab="slope")

# mean <- NULL
# for(i in 1:8){
#   mean <- rbind(mean, cbind(mean2 = apply(orig_mwu[[i]]$trans, 2, median),
#                 slope = slope[i]))
# }
# mean <- as.data.frame(mean)
# 
...
orig_GO_N <- orig_GO_N[-which(orig_GO_N$method %in% c("elim", "weight") & orig_GO_N$fdr == "fdr"),]
ggplot(data=orig_GO_N[orig_GO_N$method!="lea",], mapping=aes(x=slope, y=N,col=fdr))+
  geom_boxplot()+
  facet_wrap(~method)+
  # geom_jitter(width=0.01)+
  theme_minimal()

orig_GO_N %>% group_by(method, slope, fdr) %>% summarise(median=median(N), mean=mean(N/100), max=max(N/100), min=min(N/100)) -> sumitup
...
```{r}
ggplot(data = orig_GO_N_o_t[orig_GO_N_o_t$method=="classic",], mapping=aes(x=N_trans, y=N_sig/100, col=fdr))+
  geom_point()+
...
```{r}
ggplot(data = orig_GO_N_o_t[orig_GO_N_o_t$method=="elim",], mapping=aes(x=N_trans, y=N_sig/100, col=fdr))+
  geom_point()+
  facet_grid(fdr~slope)+
  ggtitle("elim")+
  scale_x_log10()+
...
```{r}
ggplot(data = orig_GO_N_o_t[orig_GO_N_o_t$method=="weight",], mapping=aes(x=N_trans, y=N_sig/100, col=fdr))+
  geom_point()+
  facet_grid(fdr~slope)+
   ggtitle("weight")+
    scale_x_log10()+
...
```{r}
ggplot(data = orig_GO_N_o_t[orig_GO_N_o_t$method=="mwu",], mapping=aes(x=N_trans, y=N_sig/100, col=fdr))+
  geom_point()+
  facet_grid(fdr~slope)+
  ggtitle("MWU")+
  scale_x_log10()+
...
```{r}
ggplot(data=orig_fc_o_trans[orig_fc_o_trans$method=="lea",], mapping=aes(x=log2FC, y=N, col=fdr))+
  geom_point()+
  facet_grid(fdr~slope)+
  ggtitle("Weight")+
  ylab("Number of GO Terms per transcript")+
...
c <- NULL
for(j in 1:8){
test <- get_lines(fcvals = orig_results[[j]]$classic$trans, nvals = orig_results[[j]]$classic$trans_n)
...
c_fdr <- NULL
for(j in 1:8){
test <- get_lines(fcvals = orig_results[[j]]$classic$trans, nvals = orig_results[[j]]$classic$trans_n_fdr)
plot(x=log10(1:300), test[,2], type="l", xlim=c(0,log10(100)), ylim=c(-2,2), xaxt="n",
     xlab="Number of significant GO terms", ylab="Cumulative Median log2FC", main=paste0(buchstab[j], ") ", "slope: ", slope[j]))
axis(side=1, at=log10(c(1,3,5,10,25,50,100,250,500,1000,2500,5000,10000)), labels=c(1,3,5,10,25,50,100,250,500,1000,2500,5000,10000), las=3)
for(i in 3:100){
...
e <- NULL
for(j in 1:8){
test <- get_lines(fcvals = orig_results[[j]]$elim$trans, nvals = orig_results[[j]]$elim$trans_n)
plot(x=log10(1:300), test[,2], type="l", xlim=c(0,log10(100)), ylim=c(-2,2), xaxt="n",
     xlab="Number of significant GO terms", ylab="Cumulative Median log2FC", main=paste0(buchstab[j], ") ", "slope: ", slope[j]))
axis(side=1, at=log10(c(1,3,5,10,25,50,100,250,500,1000,2500,5000,10000)), labels=c(1,3,5,10,25,50,100,250,500,1000,2500,5000,10000), las=3)
for(i in 3:100){
...
for(j in 1:8){
test <- get_lines(fcvals = orig_results[[j]]$weight$trans, nvals = orig_results[[j]]$weight$trans_n)
plot(x=log10(1:300), test[,2], type="l", xlim=c(0,log10(100)), ylim=c(-2,2), xaxt="n",
     xlab="Number of significant GO terms", ylab="Cumulative Median log2FC", main=paste0(buchstab[j], ") ", "slope: ", slope[j]))
axis(side=1, at=log10(c(1,3,5,10,25,50,100,250,500,1000,2500,5000,10000)), labels=c(1,3,5,10,25,50,100,250,500,1000,2500,5000,10000), las=3)
for(i in 3:100){
...
m <- NULL
for(j in 1:8){
test <- get_lines(fcvals = orig_mwu[[j]]$trans, nvals = orig_mwu[[j]]$trans_n)
plot(x=log10(1:300), test[,2], type="l", xlim=c(0,log10(100)), ylim=c(-2,2), xaxt="n",
     xlab="Number of significant GO terms", ylab="Cumulative Median log2FC", main=paste0(buchstab[j], ") ", "slope: ", slope[j]))
axis(side=1, at=log10(c(1,3,5,10,25,50,100,250,500,1000,2500,5000,10000)), labels=c(1,3,5,10,25,50,100,250,500,1000,2500,5000,10000), las=3)
for(i in 3:100){
...
m_fdr <- NULL
for(j in 1:8){
test <- get_lines(fcvals = orig_mwu[[j]]$trans, nvals = orig_mwu[[j]]$trans_n_fdr)
plot(x=log10(1:300), test[,2], type="l", xlim=c(0,log10(100)), ylim=c(-2,2), xaxt="n",
     xlab="Number of significant GO terms", ylab="Cumulative Median log2FC", main=paste0(buchstab[j], ") ", "slope: ", slope[j]))
axis(side=1, at=log10(c(1,3,5,10,25,50,100,250,500,1000,2500,5000,10000)), labels=c(1,3,5,10,25,50,100,250,500,1000,2500,5000,10000), las=3)
for(i in 3:100){
...
```{r}
orig_med_lines <- as.data.frame(cbind(rbind(c, c_fdr, e, w, m, m_fdr), data="original"))
orig_med_lines$log2fc <- as.numeric(orig_med_lines$log2fc)
orig_med_lines$N <- as.numeric(orig_med_lines$N)
```
...
for(j in 1:length(unique(orig_med_lines$method))){
  print(j)
  method=unique(orig_med_lines$method)[j]
  plot(log10(orig_med_lines$N[orig_med_lines$method==method & orig_med_lines$slope == slope[j]]), orig_med_lines$log2fc[orig_med_lines$method==method & orig_med_lines$slope == slope[1]], type="l", ylim=c(-2,2), xlab="Number of significant GO terms", ylab="Cumulative Median log2FC", main=paste0(buchstab[j], ") method: ", method), xaxt="n")
  ...
  for(i in 2:8){
    lines(log10(orig_med_lines$N[orig_med_lines$method==method & orig_med_lines$slope == slope[i]]), orig_med_lines$log2fc[orig_med_lines$method==method & orig_med_lines$slope == slope[i]], type="l", ylim=c(-2,2), col = colfunc(8)[i])
  }
